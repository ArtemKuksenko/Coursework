...
function loop(tableName,index,countCandles,end,activation){
	if(index<end){
		...
		calcProfit(arrGlobalOfGoodIndex,tableName,end);
	}
}
...
//run this aplication
lastIdex("tron_m15",4+1,2.5); //таблица, кол-во рассматр свечей(k -> k+1),активация
...
function calcProfit(arr,tableName,end){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// const q = "select close from" + tableName +
			  // " where period > "; 
	...
	function loop(i){
		if(i< arr.length){
			...
			gradient(profit,arr.length,end);
		}
	}
}
...
function gradient(prof,length,end){
	...
	if (porogAlpha > 0) {
		...
		profit = 0;
		lastIdex("tron_m15",4+1,porogAlpha);
	} else {
